...
function Profile() {
  ...
  const currUserFollowing = users.activeprofiles.find(
    (user) => user.id === currUser.user
  ).info.following;
  const username = params.username;
  const foundUser = users.activeprofiles.find((user) =>
    user.info !== undefined ? user.info.username === username : ""
  );
  const { id, info } = foundUser !== undefined ? foundUser : {};
  var postsFound = posts[
    chosen.toLowerCase() === "posts" ? "profile" : chosen.toLowerCase()
  ].find((posts) => posts.user === id);
      ...
      if (userPosts === undefined) {
        dispatch(
          getPost(
            undefined,
            chosen.toLowerCase() === "posts" ? "profile" : chosen.toLowerCase(),
            chosen.toLowerCase() !== "replies" &&
              chosen.toLowerCase() !== "likes"
              ? undefined
              : 0,
            username,
            currUser.token,
        ...
        dispatch(
          getPost(
            ...
            chosen.toLowerCase() !== "replies" &&
              chosen.toLowerCase() !== "likes"
              ? userPosts.posts[userPosts.posts.length - 1]
        ...
        <div className="w-full box-border px-3 h-[60px] flex items-center justify-between bg-transparent z-50">
          ...
          <div className="leading-6 w-[90%]">
            ...
            <span className="text-[#536471]">
              {postsFound !== undefined ? postsFound.posts.length : 0} posts
            </span>
          </div>
        </div>
