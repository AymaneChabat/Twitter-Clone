...
function UpdateUser({ setUpdating, user }) {
  ...
  return (
    <div
      className="absolute w-full h-full bg-[#000000]/[.7] z-50 flex justify-center items-center dark:bg-[#ffffff]/[.2] "
      onMouseDown={() => {
        setUpdating(false);
      }}
    >
      <div
        className="bg-[#ffffff] s7:w-[580px] s7:h-[620px] h-full w-full s7:rounded-lg relative dark:bg-[#000000] animate-fade-in"
        onMouseDown={(e) => {
          e.stopPropagation();
        }}
      >
        <div className="flex justify-between items-center p-2.5">
          <button
            className="hover:bg-[#000000]/[.1] p-2 rounded-full"
            onClick={() => {
              setUpdating(false);
            }}
          >
          ...
          <button
            className="bg-[#000000] text-[#ffffff] py-1 px-4 rounded-full dark:border dark:hover:bg-[#ffffff]/[.1]"
        ...
        <div
          className={
            " w-full h-[200px] flex justify-center items-center bg-no-repeat bg-cover bg-center"
          }
          style={{ backgroundImage: `url("${banner.url}")` }}
        >
          <ReactFileReader
            handleFiles={handleFilesBanner}
            multipleFiles={false}
            base64={true}
          >
          ...
          {banner.url !== user.banner ? (
            <button
              className="hover:bg-[#ffffff]/[.8] p-3 rounded-full cursor-pointer bg-[#000000]/[.5] ml-3 transition duration-300"
              onClick={() => {
                setBanner({ url: user.banner });
              }}
            >
              <DeleteIcon />
            </button>
        ...
        <div className="absolute top-[195px] w-full">
          <div
            className="w-[130px] h-[130px] rounded-full border-4 mx-5 flex justify-center items-center border-[#ffffff] bg-no-repeat bg-cover bg-center dark:border-[#000000]"
            style={{ backgroundImage: `url("${pp.url}")` }}
          >
            <ReactFileReader
              handleFiles={handleFilesPP}
              multipleFiles={false}
              base64={true}
            >
              <div className="p-3 hover:bg-[#000000]/[.2] rounded-full cursor-pointer bg-[#000000]/[.5] transition duration-300">
            ...
            {pp.url !== user.profilepicture ? (
              <button
                className="hover:bg-[#ffffff]/[.8] p-3 rounded-full cursor-pointer bg-[#ffffff]/[.5] ml-3 transition duration-300"
                onClick={() => {
                  setPP({ url: user.profilepicture });
                }}
              >
                <DeleteIcon />
              </button>
