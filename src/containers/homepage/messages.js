...
function Messages({w}) {
    ...
    <div className='h-[5%] w-full px-3 py-2'>
      <div className='flex justify-between w-full bg-[#ffffff] h-full items-center'>
        <div className='flex items-center'>
          <div className='block s11:hidden mr-3' onClick={()=>setActiveChat(null)}>
            <BackArrowIcon w={20}/>
          </div>
          {chatOverflow > 0 ? (
          <>
  ...
  return (
    <div className='s8:w-[70%] w-[100%] border-l flex mb-[60px] s5:mb-0'>
      <div className={'min-w-[380px] border-r h-full grow s11:grow-0 ' + (activeChat !== null && w < 1100 ? "hidden" : "block")}>
      ...
      <div className={'s11:w-[590px] w-full border-r h-full relative ' + (w > 1100 || activeChat !== null ? "block" : "hidden")}>
        {convoInfo ? conversationInfo({"name": "DemianRTG", "username": "DemianRTG"}) : activeChat !== null ? conversation({"name": "DemianRTG", "username": "DemianRTG"}) : ""}
      </div>
    </div>
  );
}
