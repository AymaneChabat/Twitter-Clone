...
import {
  ...
  DetailsIcon,
  BackArrowIcon,
} from "../../components/icons/messages";
...
function Messages({ w }) {
  ...
  const activeUser =
    chats.chats.find((chat) => chat.id === chats.activeChat) !== undefined
      ? users.activeprofiles.find(
          (user) =>
            user.id ===
            chats.chats
              .find((chat) => chat.id === chats.activeChat)
              .chat.participants.filter(
                (participant) => participant !== currUser.user
              )[0]
        )
      : "";
  const messages =
    chats.activeChat !== null
      ? currMessages.find((messages) => messages.chat === chats.activeChat)
      : undefined;
  ...
  useEffect(() => {
    ...
    if (messageDiv) {
      messageDiv.addEventListener("keydown", (event) => {
        ...
        if (keyCode === 13) {
          ...
          event.textContent += sendButton.click();
        }
      });
    }
    ...
    const q = query(
      collection(db, "chats"),
      where("participants", "array-contains", currUser.user)
    );
    onSnapshot(q, () => {
  ...
  useEffect(() => {
    ...
    if (chats.activeChat !== null) {
      if (chats.activeChat.slice(0, 6) !== "sample") {
        const q = query(
          collection(db, "chats/" + chats.activeChat + "/messages")
        );
  ...
  function formatCustomDate(timestamp) {
    ...
    if (
      ...
      return (
        formattedProvidedData[2] +
        ", " +
        providedDate.toTimeString().split(" ")[0]
      );
      ...
      return (
        formattedProvidedData[1] +
        "/" +
        formattedProvidedData[2] +
        "/" +
        formattedProvidedData[3]
      );
    }
  }
  ...
  const conversation = (data) => (
    <Fragment>
      <div
        className="h-[95%] overflow-auto shrink transition-all duration-300 px-3"
        id="chat"
      ...
      >
        <div className="w-full py-2 h-[5%] sticky top-0 bg-transparent bg-blur backdrop-blur-sm">
          <div className="flex justify-between w-full h-full items-center">
            <div className="flex items-center">
              ...
              {chatOverflow > 0 && (
                <>
                  ...
                  <span className="font-semibold tracking-tight ml-2 dark:text-[#ffffff]">
                    {data.info.name}
                  </span>
                </>
              )}
            ...
            <div
              className="p-1.5 hover:bg-[#000000]/[.1] rounded-full cursor-pointer"
              onClick={() => {
                setConvoInfo(true);
              }}
            >
        ...
        <div className="h-auto w-full flex flex-col items-center mb-8">
          <div className="p-4 w-full">
            <div className="w-full flex justify-center">
              <div
                className="h-[50px] w-[50px] rounded-full bg-cover bg-no-repeat bg-center"
                style={{
                  backgroundImage: `url("${data.info.profilepicture}")`,
                }}
              ></div>
            </div>
            <h1 className="text-[#000000] mr-1 font-semibold tracking-tight w-full text-center dark:text-[#ffffff]">
              {data.info.name}
            </h1>
            <h3 className="text-[#536471] text-[16px] tracking-tighter w-full text-center">
              @{data.info.username}
            </h3>
          </div>
          <p className="font-twitterchirp text-[14px] w-[70%] text-center dark:text-[#ffffff]">
            {data.info.description}
          </p>
          <span className="text-[#536471] text-[14px] tracking-tighter mt-2">
            Joined May 2018 · 3,551 Followers
          </span>
        </div>
        {!loading ? (
          messages &&
          messages.messages.map((message, index) =>
            message.message.sender === currUser.user
              ? Sent(message, index)
        ...
        <div className="flex py-2.5 items-center px-4 grow bg-[#ffffff] dark:bg-transparent">
          <div className="grow relative">
            <div
              ...
              className="dark:text-[#ffffff] bg-transparent focus:outline-none text-sm text-[#0f1419] grow break-word"
              onInput={(e) => {
            ...
            {images !== "" ? (
              <div className="relative h-[150px] w-[150px] mt-2">
                <div
                  className="absolute text-center bg-[#ffffff] rounded-full p-1 right-1 top-1 hover:bg-[#000000]/[.7] cursor-pointer"
                  onClick={() => {
                    setImages("");
                  }}
                >
            ...
            {newMessage === "" && images === "" && (
              <div className="text-[#000000]/[.6] text-sm font-chirp absolute pointer-events-none top-0 dark:text-[#ffffff]/[.6]">
                Start a new message
              </div>
            )}
          </div>
          <div
            ...
            className={
              newMessage === ""
                ? "pointer-events-none cursor-not-allowed ml-2"
                : "cursor-pointer ml-2"
            }
            onClick={() => {
    ...
    <Fragment>
      <div className="h-[5%] w-full py-2">
        <div className="px-3 flex w-full h-full items-center">
          <div
            className="rounded-full hover:bg-[#000000]/[.1] p-1.5 cursor-pointer"
            onClick={() => {
              setConvoInfo(false);
            }}
          >
          ...
          <span className="font-bold ml-4 text-[20px] dark:text-[#ffffff]">
            Conversation info
          </span>
        </div>
      </div>
      <div className="flex items-center hover:bg-blue-200/[.1] px-3 py-5 mt-4 cursor-pointer border-b border-[#536471]/[.1] transition-all duration-300">
        <div className="w-[11%]">
          <div
            className="w-[45px] h-[45px] rounded-full bg-cover bg-no-repeat bg-center"
            style={{ backgroundImage: `url("${data.info.profilepicture}")` }}
          ></div>
        </div>
        <div className="w-[69%]">
          <div className="flex flex-col leading-6">
            <span className="text-[#000000] mr-1 font-semibold tracking-tight hover:underline dark:text-[#ffffff]">
              {data.info.name}
            </span>
            <span className="text-[#536471] font-chirp text-md tracking-tighter">
              @{data.info.username}
            </span>
          </div>
        </div>
        <div className="w-[20%]">
          <button className="bg-[#000000] text-[#ffffff] font-twitterchirp font-bold py-2 px-5 rounded-full hover:bg-[#000000]/[.8] dark:bg-[#ffffff] dark:text-[#000000] dark:hover:brightness-75 transition-all duration-300">
            Follow
          </button>
        </div>
      </div>
      <div className="p-3 border-b border-[#536471]/[.1]">
        <h1 className="font-bold dark:text-[#ffffff] text-[23px]">
          Notifications
        </h1>
        <div className="flex items-center justify-between mt-4 mb-2">
          <span className="text-[#536471] font-chirp text-lg tracking-tighter">
            Snooze notifications from &nbsp;&nbsp;{data.info.name}
          </span>
      ...
      <button className="text-center py-4 hover:bg-[#1d9bf0]/[.1] cursor-not-allowed w-full transition-all duration-300">
        <span className="text-[#1d9bf0] text-[18px] font-semibold">
          Block @{data.info.username}
        </span>
      </button>
      <Link to={"/messages"}>
        <button
          className="text-center py-4 hover:bg-[#f4212e]/[.1] cursor-pointer w-full transition-all duration-300"
          onClick={() => {
            setConvoInfo(false);
            dispatch(removeChat(currUser.token, chats.activeChat));
          }}
        >
          <span className="text-[#f4212e] text-[18px] font-semibold text-center">
            Leave Conversation
          </span>
        </button>
      </Link>
  ...
  return (
    <section className="s8:w-[70%] w-[100%] border-l flex mb-[60px] s6:mb-0 max-h-[93%] s6:max-h-[100%] s6:h-auto border-[#1d9bf0]/[.1] dark:border-[#ffffff]/[.3]">
      <div
        className={
          "s7:w-[380px] border-r h-full grow s11:grow-0 border-[#1d9bf0]/[.1] dark:border-[#ffffff]/[.3] " +
          (chats.activeChat !== null && w < 1100 ? "hidden" : "block")
        }
      >
        <div className="flex p-3 justify-between items-center">
          <span className="text-[18px] font-bold dark:text-[#ffffff]">
            Messages
          </span>
        ...
        <div className="my-4 px-3">
          <div
            className={
              "flex py-2.5 my-2 items-center rounded-full border px-4 bg-[#ffffff] dark:bg-[#16181c] dark:border-[#000000] " +
              (focused ? "border-[#1d9bf0] " : "")
            }
          >
            <SearchIcon
              picked={[false, "Explore"]}
              color={focused ? "#1d9bf0" : "#808080"}
              size={"16"}
            />
            <input
              onChange={(e) => {
              ...
              className="bg-transparent ml-3 focus:outline-none text-sm text-[#0f1419] dark:text-[#ffffff]"
            />
          </div>
        </div>
        <div className="messages">
          {chats.chats.length > 0 &&
            chats.chats.map((chat, index) => {
              const user = users.activeprofiles.find(
                (user) =>
                  user.id ===
                  chat.chat.participants.filter(
                    (participant) => participant !== currUser.user
                  )[0]
              );
              return (
                <Link to={"/messages/" + chat.id}>
                  <div
                    className={
                      "cursor-pointer w-full border-[#1d9bf0] p-3 flex items-center " +
                      (chat.id === chats.activeChat
                        ? "border-r bg-[#97d0f7]/[.1]"
                        : "")
                    }
                    key={index}
                    id={chat.id}
                    ...
                    <div className="leading-6 block">
                      <div className="flex">
                        <div className="s7:max-w-[80px] max-w-[140px] overflow-hidden relative block text-ellipsis whitespace-nowrap dark:text-[#ffffff]">
                          <span className="text-[#000000] mr-1 tracking-tight dark:text-[#ffffff] ">
                            {user.info.name}
                          </span>
                        </div>
                        ...
                        <div className="overflow-hidden relative block text-ellipsis whitespace-nowrap">
                          <span className="text-[#536471] font-chirp tracking-tighter">
                            {" "}
                            · {formatCustomDate(chat.chat.updatedAt)}
                          </span>
                        </div>
                      </div>
                      <div className="s7:max-w-[300px] max-w-[350px] overflow-hidden relative block text-ellipsis whitespace-nowrap">
                        <span className="text-[15px] dark:text-[#ffffff]">
                          hello
                        </span>
                      </div>
                    </div>
      ...
      <div
        className={
          "s11:w-[590px] w-full border-r h-full relative border-[#1d9bf0]/[.1] dark:border-[#ffffff]/[.3] " +
          (w > 1100 || chats.activeChat !== null ? "block" : "hidden")
        }
      >
        {activeUser.id !== undefined &&
          (convoInfo
            ? conversationInfo(activeUser)
            : chats.activeChat !== null
