  ...
  <Fragment>  
    ...
    <div className='w-full min-h-[5%] bottom-0 flex items-center border-t grow absolute bg-[#ffffff]'>
      ...
      <div className='flex py-2.5 items-center px-4 grow bg-[#ffffff]'>
        ...
        <div className={newMessage === "" ? 'pointer-events-none cursor-not-allowed ml-2' : "cursor-pointer ml-2"} onClick={()=>{dispatch(sendMessage(currUser.token, {chatId: chats.activeChat, content: newMessage, media: []})); setNewMessage(""); document.querySelector("#newMessage").innerHTML = "";}}>
          <SendIcon w={18}/>
        </div>
      </div>
    </div>
    ...
    <div className={'s7:w-[380px] border-r h-full grow s11:grow-0 ' + (chats.activeChat !== null && w < 1100 ? "hidden" : "block")}>
        ...
        <div className='messages'>
          {chats.last !== undefined ? chats.chats.map((chat, index)=>{
            const user = users.activeprofiles.find(user => user.id === chat.chat.participants.filter(participant => participant !== currUser.uid)[0])
            return(
