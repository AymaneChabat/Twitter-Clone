...
const ConversationInfo = ({data, currUser, activeChat}) => {
  ...
  <Fragment>
    <div className="h-[5%] w-full py-2">
      <div className="px-3 flex w-full h-full items-center">
        <Link className="rounded-full hover:bg-[#000000]/[.1] p-1.5 cursor-pointer" onClick={()=>{navigate(-1)}}>
          <BackArrowIcon w={20} />
        </Link>
    ...
    <Link to={"/messages"}>
    <button className="text-center py-4 hover:bg-[#f4212e]/[.1] cursor-pointer w-full transition-all duration-300" onClick={()=> { dispatch(removeChat(currUser.token, activeChat)); }} > <span className="text-[#f4212e] text-[18px] font-semibold text-center"> Leave Conversation </span>
    </button>
    </Link>
  </Fragment>
)};

const Conversation = ({data, currUser, activeChat}) => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  ...
  const messages =
    activeChat !== null
      ? currMessages.find((messages) => messages.chat === activeChat)
      : undefined;
  ...
  useEffect(() => {
    if (messages === undefined && activeChat !== undefined) {
      dispatch(fetchMessages(currUser.token, activeChat));
    }

    let initialSnapshot = true; // Initialize the flag

    if (activeChat !== undefined) {
      if (activeChat.slice(0, 6) !== "sample") {
        const q = query(
...
const ChatList = ({chats, users, currUser, activeChat}) => {
  const w = useMediaQuery('(max-width:1100px)')
  const [search, setSearch] = useState("");
  const [focused, setFocused] = useState(false);

  return (
    <div className={ "s7:w-[380px] border-r h-full grow s11:grow-0 border-[#1d9bf0]/[.1] dark:border-[#ffffff]/[.3]" + (activeChat !== null && w ? "hidden" : "block" ) }>
      ...
      <div className="messages"> {chats.chats.length > 0 && chats.chats.map((chat, index) => { const user = users.activeprofiles.find( (user) => user.id === chat.chat.participants.filter( (participant) => participant !== currUser.user )[0] ); return (
        <Link to={"/messages/" + chat.id}>
        <div className={ "cursor-pointer w-full border-[#1d9bf0] p-3 flex items-center " + (chat.id === activeChat ? "border-r bg-[#97d0f7]/[.1]" : "" ) } key={index} id={chat.id}>
          <div className="mr-2">
...
function Messages() {
  ...
  const activeUser =
    chats.chats.find((chat) => chat.id === params.chat) !== undefined
      ? users.activeprofiles.find(
          (user) =>
            user.id ===
            chats.chats
              .find((chat) => chat.id === params.chat)
  ...
  return (
    <div className="flex h-full">
      <ChatList chats={chats} currUser={currUser} users={users} activeChat={params.chat}/>
      <div
        className={
          "s11:w-[590px] w-full border-r h-full relative border-[#1d9bf0]/[.1] dark:border-[#ffffff]/[.3] " +
          (!w || params.chat ? "block" : "hidden")
        }
      >
        {activeUser.id !== undefined &&
          (params.chat && (location.pathname.split("/")[location.pathname.split("/").length-1] === "info" ? <ConversationInfo activeChat={params.chat} currUser={currUser} data={activeUser} />
            : <Conversation currUser={currUser} data={activeUser} activeChat={params.chat}/>)
            )}
      </div>
    </div>
  );
