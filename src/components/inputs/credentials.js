...
const CredentialInput = forwardRef(({reff, placeholder, password, textarea, defaultVal}) => {
    ...
    return (
        <div className={'relative '+ (textarea ? "h-[150px]" : "h-[65px]") + ' border-[3px] px-2 py-2 rounded-lg ' + (focused ? "border-[#1d9bf0]" : "")}>
            ...
            <textarea className='text-[18px] outline-none w-[95%] absolute resize-none bottom-0 h-[80%]' onFocus={()=>{setFocused(true)}} onBlur={()=>{setFocused(false)}} ref={reff} onChange={(e)=>setValue(e.currentTarget.value)} maxLength={200}>{defaultVal !== undefined ? defaultVal : ""}</textarea>}
        </div>
    )
})
