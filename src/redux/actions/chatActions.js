...
import {
  ...
  export const addSampleChat =
    (token, currUser, participant, sample, navigate) => (dispatch) => {
      // Check if a chat with the given participant exists
      checkChat(token, participant)
        .then((res) => {
          if (res.id !== undefined) {
  ...
  export const createChat =
    (token, user, sample, navigate, message) => (dispatch) => {
      // Create a new chat
      createChats(token, user)
        .then((newChat) => {
